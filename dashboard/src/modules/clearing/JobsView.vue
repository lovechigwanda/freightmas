<template>
	<div>
		...
		<div class="sd-card">
			<div v-if="loading"><div class="cc-row-skeleton cc-skeleton" v-for="i in 8" :key="i"></div></div>
			<div v-else-if="error" class="sd-state" style="color: var(--sd-red)">{{ error }}</div>
			<template v-else>
				<table class="sd-table" v-if="jobs.length">
					<thead>
						<tr>
							<th>Job</th><th>Customer</th><th>Route</th><th>Shipping Line / BL</th><th>ETA / ATA</th><th>Status</th><th>Progress</th>
						</tr>
					</thead>
					<tbody>
						<tr v-for="job in jobs" :key="job.name" :class="{ 'cc-row-overdue': job.is_overdue }">
							<td>
								<div class="sd-cell-linkgroup">
									...
									<DeskLink doctype="Clearing Job" :name="job.name" icon-only />
								</div>
							</td>
							<td>{{ job.customer }}</td>
				...
				<EmptyState v-else :icon="SearchX" title="No clearing jobs match these filters" sub="Try clearing the search or filters above." />
...
async function load() {
	...
	try {
		const res = await api.getJobs({
			search: search.value, status: status.value, direction: direction.value,
			limit_start: page.value * pageSize, limit_page_length: pageSize,
		});
		jobs.value = res.jobs;
		totalCount.value = res.total_count;
	} catch (e) {
		error.value = e.message || "Failed to load clearing jobs.";
	} finally {
		loading.value = false;
	}
}
...
const exportHref = computed(() =>
	exportUrl("jobs", { search: search.value, status: status.value, direction: direction.value })
);
