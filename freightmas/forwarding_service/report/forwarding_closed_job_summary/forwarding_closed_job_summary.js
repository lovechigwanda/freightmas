...
frappe.query_reports["Forwarding Closed Job Summary"] = {
	"filters": [
		{
	...
	onload: function(report) {
		report.page.add_inner_button(__("Export to Excel"), function() {
			...
			const url = `/api/method/freightmas.api.export_report_to_excel?report_name=Forwarding Closed Job Summary&filters=${query}`;
			window.open(url);
		}, __("Export"));
		...
		report.page.add_inner_button(__("Export to PDF"), function() {
			const filters = report.get_filter_values(true);
			const query = encodeURIComponent(JSON.stringify(filters));
			const url = `/api/method/freightmas.api.export_report_to_pdf?report_name=Forwarding Closed Job Summary&filters=${query}`;
			window.open(url);
		}, __("Export"));
