...
def export_truck_trip_summary_to_pdf(report_name, filters):
    ...
    def parse_currency(value):
        ...
        if isinstance(value, (int, float)):
            return float(value)
        # Handle currency string formatting
        try:
            if isinstance(value, str):
                # Remove currency symbol, commas, spaces and handle negative values
                clean_value = value.replace('$', '').replace(',', '').replace(' ', '')
                if '(' in clean_value and ')' in clean_value:  # Handle parentheses negative numbers
    ...
    for row in data:
        row['estimated_revenue'] = parse_currency(row.get('estimated_revenue'))
    ...
    def format_filter_value(key, value):
        if 'date' in key and not isinstance(value, str):
            return frappe.utils.formatdate(value, "dd-MMM-yy")
        return value
    ...
    for key, value in filters.items():
        if value:
            ...
            filters_list.append(f"{label}: {formatted_value}")
    
    filters_html = " | ".join(filters_list)

    # Get available trucks only if no truck filter is applied
    available_trucks = []
    if not filters.get('truck'):  # Only get available trucks if no truck filter
    ...
    context = {
        ...
        "show_available_trucks": not filters.get('truck'),  # Add flag to control display
        "frappe": frappe,
        "company": frappe.defaults.get_user_default("Company"),
        "exported_at": frappe.utils.now_datetime().strftime("%d-%b-%Y %H:%M")
    }
...
def export_truck_trip_summary_to_excel(report_name, filters):
    ...
    # Add available trucks section only if no truck filter is applied
    if not filters.get('truck'):
        used_trucks = list(set([row.get('truck') for row in data if row.get('truck')]))
        available_trucks = frappe.get_all(
            "Truck",
            filters={
...
########################################################
## Statement of Accounts Allocation PDF Export
@frappe.whitelist()
def export_statement_of_accounts_allocation_to_pdf(filters):
    """Export Statement of Accounts Allocation to PDF"""
    check_freightmas_role()
    import json
    ...
    module = frappe.get_module(
        "freightmas.freightmas.report.statement_of_accounts_allocation.statement_of_accounts_allocation"
    )
    columns, data = module.execute(filters)
    ...
    context = {
        ...
        "party_type": party_type,
        "party_name": party_name or party,
        "report_name": "Statement of Accounts Allocation",
        "from_date": format_date(filters.get('from_date')),
        "to_date": format_date(filters.get('to_date')),
        "currency": company_currency,
        "data": data,
    ...
    html = frappe.render_template(
        "freightmas/templates/statement_of_accounts_allocation.html",
        context
    )
    ...
    frappe.local.response.filename = get_report_filename("Statement_of_Accounts_Allocation", "pdf", party)
    frappe.local.response.filecontent = pdf
    frappe.local.response.type = "download"


####################################################
# Export Statement of Accounts Allocation to Excel
@frappe.whitelist()
def export_statement_of_accounts_allocation_to_excel(filters):
    """Export Statement of Accounts Allocation to Excel"""
    check_freightmas_role()
    import json
    from io import BytesIO
    ...
    ws.merge_cells(f'A{current_row}:{last_col}{current_row}')
    ws[f'A{current_row}'] = "Statement of Accounts Allocation"
    ws[f'A{current_row}'].font = subtitle_font
    current_row += 1
    ...
    frappe.local.response.filename = get_report_filename("Statement_of_Accounts_Allocation", "xlsx", party)
    frappe.local.response.filecontent = output.read()
    frappe.local.response.type = "binary"
##########################################################
...
def export_statement_of_account_job_linked_to_excel(filters):
    ...
    from freightmas.utils.statement_job_linked_export import build_statement_job_linked_excel
