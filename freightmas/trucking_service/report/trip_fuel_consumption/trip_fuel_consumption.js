...
frappe.query_reports["Trip Fuel Consumption"] = {
	"filters": [
		{
	...
	onload: function(report) {
		report.page.add_inner_button('Export to Excel', function() {
			...
			const url = `/api/method/freightmas.api.export_report_to_excel?report_name=Trip Fuel Consumption&filters=${query}`;
			window.open(url);
		}, 'Export');
		...
		report.page.add_inner_button('Export to PDF', function() {
			const filters = report.get_filter_values(true);
			const query = encodeURIComponent(JSON.stringify(filters));
			const url = `/api/method/freightmas.api.export_report_to_pdf?report_name=Trip Fuel Consumption&filters=${query}`;
			window.open(url);
		}, 'Export');
	...
	formatter: function(value, row, column, data, default_formatter) {
		...
		if (column.fieldname == "fuel_variance") {
			const variance = data.fuel_variance;
			if (variance < 0) {  // Actual fuel used is less than expected
				value = `<span style="color: green">${value}</span>`;
			} else if (variance > 0) {  // Actual fuel used is more than expected
