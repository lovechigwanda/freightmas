...
frappe.query_reports["Truck Trip Summary"] = {
	onload: function(report) {
		...
		report.page.add_inner_button('Clear Filters', function() {
			...
			// Trigger a refresh after clearing filters
			report.refresh();
		});
	},
			...
			"on_change": function () {
				...
				switch (date_range) {
					...
					case "This Year":
						...
						break;
					case "Last Year":
						let last_year = (new Date(today)).getFullYear() - 1;
	...
	"formatter": function(value, row, column, data, default_formatter) {
		...
		if (data.is_total) {
			if (column.fieldname == "route" || 
				column.fieldname == "customer" || 
				column.fieldname == "estimated_revenue") {
				// Wrap in bold for web view
	...
	// Add PDF/Excel formatting options
	"pdf_report_options": {
		"total_row_style": {
			"font-weight": "bold"
		}
