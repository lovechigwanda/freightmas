...
# ----------------------------------------
# Dynamic Filter: Job No (Only with unpurchased charges)
# ----------------------------------------

@frappe.whitelist()
def get_job_nos_with_unpurchased_charges(doctype, txt, searchfield, start, page_len, filters):
    return frappe.db.sql("""
        SELECT DISTINCT cj.name
        FROM `tabClearing Job` cj
        JOIN `tabClearing Charges` rc ON rc.parent = cj.name
        WHERE rc.buy_rate > 0 AND (rc.is_purchased = 0 OR rc.is_purchased IS NULL)
...
# ----------------------------------------
# Dynamic Filter: Currency (Only for jobs with unpurchased charges)
# ----------------------------------------

@frappe.whitelist()
def get_currencies_with_unpurchased_charges(doctype, txt, searchfield, start, page_len, filters):
    return frappe.db.sql("""
