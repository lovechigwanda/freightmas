...
frappe.query_reports["Statement of Accounts Allocation"] = {
    onload: function(report) {
        report.page.add_inner_button('Export to Excel', function() {
            const filters = report.get_filter_values(true);
            const query = encodeURIComponent(JSON.stringify(filters));
            const url = `/api/method/freightmas.api.export_statement_of_accounts_allocation_to_excel?filters=${query}`;
            window.open(url);
        }, 'Export');
        ...
        report.page.add_inner_button('Export to PDF', function() {
            const filters = report.get_filter_values(true);
            const query = encodeURIComponent(JSON.stringify(filters));
            const url = `/api/method/freightmas.api.export_statement_of_accounts_allocation_to_pdf?filters=${query}`;
            window.open(url);
        }, 'Export');
    ...
    "filters": [
        ...
        {
            "fieldname": "show_opening_balance",
            "label": __("Show Opening Balance"),
            "fieldtype": "Check",
            "default": 1
        }
    ]
