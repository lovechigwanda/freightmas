...
frappe.query_reports["Creditors Listing"] = {
  filters: [
    {
      fieldname: "company",
      label: __("Company"),
  ...
  onload: function (report) {
    report.page.add_inner_button(
      ...
      function () {
        ...
        const url =
          `/api/method/freightmas.api.export_report_to_excel` +
          `?report_name=Creditors Listing` +
          `&filters=${query}`;
        window.open(url);
      },
      __("Export")
    ...
    report.page.add_inner_button(
      ...
      function () {
        ...
        const query = encodeURIComponent(JSON.stringify(filters));
        const url =
          `/api/method/freightmas.api.export_report_to_pdf` +
          `?report_name=Creditors Listing` +
          `&filters=${query}`;
        window.open(url);
      },
