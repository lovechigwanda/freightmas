...
frappe.query_reports["FM Accounts Receivable Summary"] = {
    ...
    formatter: function (value, row, column, data, default_formatter) {
        ...
        // Highlight draft amounts in blue
        if (column.fieldname === "outstanding_draft" && data.outstanding_draft > 0) {
            value = `<span style="color:#3498db; font-style:italic">${value}</span>`;
        }
    ...
    onload: function (report) {
        ...
        report.page.add_inner_button(__('Export to Excel'), function () {
            ...
            const url = `/api/method/freightmas.api.export_report_to_excel`
                + `?report_name=FM Accounts Receivable Summary`
                + `&filters=${query}`;
            window.open(url);
        }, __('Export'));
