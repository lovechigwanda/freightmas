...
frappe.query_reports["FM Accounts Receivable Outstanding"] = {
    "filters": [
        {
            "fieldname": "company",
            "label": __("Company"),
    ...
    "onload": function(report) {
        ...
        report.page.add_inner_button('Export to Excel', function() {
            ...
            const url = `/api/method/freightmas.api.export_report_to_excel?report_name=FM Accounts Receivable Outstanding&filters=${query}`;
            window.open(url);
        }, 'Export');
        ...
        report.page.add_inner_button('Export to PDF', function() {
            const filters = report.get_filter_values(true);
            const query = encodeURIComponent(JSON.stringify(filters));
            const url = `/api/method/freightmas.api.export_report_to_pdf?report_name=FM Accounts Receivable Outstanding&filters=${query}`;
            window.open(url);
        }, 'Export');
