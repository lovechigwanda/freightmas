...
// ====================================================
// This script controls the calculations and currency handling for Road Freight Job
// Similar to the logic implemented in Clearing Job
// ====================================================
...
function update_currency_labels(frm) {
  ...
  if (frm.fields_dict.road_freight_charges) {
    frm.fields_dict.road_freight_charges.grid.update_docfield_property("sell_rate", "label", `Sell Rate (${currency})`);
    frm.fields_dict.road_freight_charges.grid.update_docfield_property("buy_rate", "label", `Buy Rate (${currency})`);
    frm.fields_dict.road_freight_charges.grid.update_docfield_property("revenue_amount", "label", `Revenue (${currency})`);
    frm.fields_dict.road_freight_charges.grid.update_docfield_property("cost_amount", "label", `Cost (${currency})`);
  }
}
...
frappe.ui.form.on('Road Freight Charges', {
  ...
  road_freight_charges_remove(frm) {
    calculate_freight_totals(frm);
  }
});
