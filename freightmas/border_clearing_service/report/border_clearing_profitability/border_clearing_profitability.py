...
def execute(filters=None):
    ...
    if filters.get("border_post"):
        ...
        params["border_post"] = filters["border_post"]

    where_clause = " AND ".join(conditions)

    jobs = frappe.db.sql("""
        SELECT name, date_created, customer, customer_reference, direction, border_post, status,
    ...
    for job in jobs:
        ...
        invoiced_revenue = frappe.db.sql("""
            ...
            WHERE docstatus = 1 AND border_clearing_job_reference = %s
        """, (job["name"]))[0][0] or 0
        ...
        data.append({
            ...
            "border_post": job.get("border_post", ""),
